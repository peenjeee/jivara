...
const SplashScreenContext = createContext<SplashScreenContextValue>({ isSplashFinished: false });
...
export default function AppSplashScreen() {
  ...
  useEffect(() => {
    ...
    const readyTimer = window.setTimeout(() => setIsReady(true), splashDuration);
  ...
  return (
    <SplashScreenContext.Provider value={{ isSplashFinished: isReady }}>
      <SplashOverlay isReady={isReady} isStandalonePwa={isStandalonePwa} />
    </SplashScreenContext.Provider>
  );
}

function SplashOverlay({ isReady, isStandalonePwa }: { readonly isReady: boolean; readonly isStandalonePwa: boolean }) {
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    if (!isStandalonePwa || isReady) return;
  ...
  }, [isStandalonePwa, isReady]);

  return (
    <>
      {!isReady && (
        <div className="fixed inset-0 z-[99999] bg-white pointer-events-none" aria-hidden="true" />
      )}
      <AnimatePresence>
        {isVisible && (
          <motion.div
